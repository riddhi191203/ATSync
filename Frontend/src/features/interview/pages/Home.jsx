...
const Home = () => {
  ...
  const updateFile = (nextFile) => {
    const message =
      !nextFile ? 'No file selected.' :
      !isPdf(nextFile.name) ? 'Only PDF files are supported.' :
      nextFile.size > MAX_FILE_SIZE ? 'File must be 5MB or smaller.' :
      ''
  ...
  const submit = async () => {
    const resume = file || resumeRef.current?.files?.[0]
    if (!resume && !selfDescription.trim()) {
      setError('Please upload a PDF file or enter a self-description.')
      return
    }
    ...
    try {
      ...
      setError(err?.response?.data?.message || err?.message || 'Unable to upload the file. Please try again.')
    }
  }

  if (loading) return <main className='loading-screen'><h1>Loading your interview plan...</h1></main>
          ...
          <div className='panel panel--right'>
            ...
            <div className='upload-section'>
              ...
              <label
                ...
                htmlFor='resume'
                onDrop={(e) => { e.preventDefault(); setDragActive(false); updateFile(e.dataTransfer?.files?.[0]) }}
              ...
              >
                ...
                <input ref={resumeRef} hidden type='file' id='resume' name='resume' accept='.pdf' onChange={(e) => updateFile(e.target.files?.[0])} />
              </label>
